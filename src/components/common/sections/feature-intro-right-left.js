...
const HowToUseGuide = ({ data, theme = 'normal' }) => {
  return (
    <div className="bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 py-24 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 text-center relative z-10">
        <h2 className="text-4xl font-bold mb-4 text-white">
          {data?.title || "How to Use Our Image to Video Generator"}
        </h2>
        
        <p className="text-gray-300 mb-12 max-w-3xl mx-auto">
          {data?.description || "With only three steps, you can easily create videos from images with Pollo AI."}
        </p>
        
        <div className="grid md:grid-cols-3 gap-8">
          {/* Video Tutorial */}
          <div className="md:col-span-2">
            <div className="rounded-xl overflow-hidden border border-slate-700/50 shadow-xl shadow-cyan-500/10 bg-black/30 backdrop-blur-sm">
              <div className="aspect-w-16 aspect-h-9 relative">
                {/* YouTube Video Thumbnail with Play Button */}
                <div className="w-full h-full bg-slate-900 relative">
                  {/* This is a placeholder for the YouTube video */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="absolute inset-0 bg-gradient-to-r from-slate-900/70 via-transparent to-slate-900/70"></div>
                    
                    {/* Video Title and Thumbnail Content */}
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-full h-full relative">
                        {/* Stylized Image to Video Title */}
                        <div className="absolute top-1/4 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-3/4 text-left">
                          <div className="text-2xl font-bold text-white mb-1">How to Convert</div>
                          <div className="flex items-center gap-2">
                            <span className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-lime-300 to-lime-400">Image</span>
                            <span className="text-xl text-white">to</span>
                            <span className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-300 to-cyan-400">Video</span>
                          </div>
                          <div className="text-xl text-white mt-1">with Pollo AI</div>
                        </div>
                        
                        {/* Right side person explaining */}
                        <div className="absolute right-0 bottom-0 h-3/4 w-1/2 flex items-end">
                          <div className="w-full h-full bg-gradient-to-t from-slate-900/80 to-transparent"></div>
                        </div>
                        
                        {/* Image example in the center-left */}
                        <div className="absolute bottom-1/3 left-1/4 h-24 w-20 border-2 border-rose-500/70">
                          <div className="w-full h-full bg-slate-800"></div>
                        </div>
                        
                        {/* Arrow pointing right */}
                        <div className="absolute bottom-1/3 left-[35%] text-rose-500 text-2xl">&rarr;</div>
                        
                        {/* Result video thumbnail */}
                        <div className="absolute bottom-1/3 left-[45%] h-32 w-28 border-2 border-rose-500/70">
                          <div className="w-full h-full bg-slate-800"></div>
                        </div>
                    ...
                    {/* Play Button */}
                    <button className="relative z-10 w-16 h-16 bg-red-600 rounded-full flex items-center justify-center shadow-lg">
                      <div className="w-0 h-0 border-y-8 border-y-transparent border-l-12 border-l-white ml-1"></div>
                      
                      {/* YouTube branding */}
                      <div className="absolute bottom-0 left-0 transform translate-y-full mt-4 flex items-center gap-2">
                        <div className="bg-white/10 backdrop-blur-sm rounded px-2 py-1 text-xs text-white flex items-center">
                          Watch on
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-12 ml-1 text-white" viewBox="0 0 90 20">
          ...
          <div className="md:col-span-1 flex flex-col gap-4">
            ...
            <div className="bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 rounded-lg p-5 text-left">
              ...
              <p className="text-gray-300 text-sm">Upload your image.</p>
            </div>
            
            {/* Step 2 */}
            <div className="bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 rounded-lg p-5 text-left">
              <h3 className="text-lg font-medium text-white mb-2">Step 2</h3>
              <p className="text-gray-300 text-sm">Input your text prompt and set the additional customization settings.</p>
            </div>
            
            {/* Step 3 */}
            <div className="bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 rounded-lg p-5 text-left">
              <h3 className="text-lg font-medium text-white mb-2">Step 3</h3>
              <p className="text-gray-300 text-sm">View and share the generated video.</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12">
          <button className="px-8 py-3.5 rounded-full relative overflow-hidden group">
            ...
            <span className="relative text-white font-medium">Try Our Image to Video Generator</span>
          </button>
        </div>
      </div>
    </div>
