  ...
  useEffect(() => {
    ...
    if (updatedTabs.length === 1 && isFirstTimeUser) {
      ...
      if (browserTabs.length === 1 && isFirstTimeUser) {
        const modalContainer = document.createElement('div');
        modalContainer.className = 'fixed inset-0 z-[1000] flex items-center justify-center bg-black bg-opacity-75 backdrop-blur-sm p-4';
        
        const modalContent = document.createElement('div');
        modalContent.className = 'bg-gradient-to-b from-slate-900 to-slate-950 rounded-lg shadow-xl p-6 max-w-md w-full border border-blue-500/50 relative animate-fadeIn';
        
        const title = document.createElement('h3');
        title.className = 'text-xl font-semibold text-white mb-4 flex items-center gap-2';
        title.innerHTML = `
          <span>Page Generated & Colors Extracted!</span>
          <span class="text-2xl">🎨</span>
        `;
        
        const description1 = document.createElement('p');
        description1.className = 'text-gray-300 mb-3';
        description1.textContent = 'We\'ve successfully generated your first page!';
        
        const description2 = document.createElement('p');
        description2.className = 'text-gray-300 mb-3';
        description2.innerHTML = `We automatically extracted brand colors from your website URL (<strong>${userInput || 'your website'}</strong>). You can review and adjust these colors in the next step to ensure future pages match your brand perfectly.`;
        
        const description3 = document.createElement('p');
        description3.className = 'text-gray-300 mb-4 font-medium';
        description3.innerHTML = '<strong>Saving your preferred colors will also update the page already generated.</strong>';
        
        const description4 = document.createElement('p');
        description4.className = 'text-gray-400 mb-6 text-sm';
        description4.textContent = 'Click the button below to proceed to color customization.';
        
        const buttonContainer = document.createElement('div');
        buttonContainer.className = 'flex justify-center';
        
        const okButton = document.createElement('button');
        okButton.className = 'px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-md hover:from-blue-500 hover:to-indigo-500 transition-all duration-300 font-medium shadow-lg hover:shadow-blue-500/20 flex items-center gap-2';
        okButton.innerHTML = `
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M4 2a2 2 0 00-2 2v11a3 3 0 106 0V4a2 2 0 00-2-2H4zm1 14a1 1 0 100-2 1 1 0 000 2zm5-1.757l4.9-4.9a2 2 0 000-2.828l-3.172-3.172a2 2 0 00-2.828 0L8 8.243V16h2v-1.757z" clip-rule="evenodd" />
          </svg>
          Customize Colors
        `;
        ...
        okButton.onclick = () => {
          document.body.removeChild(modalContainer);
          console.log("[DEBUG] Custom modal OK button clicked. Showing BrandAssetsModal.");
        ...
        modalContent.appendChild(description3);
        modalContent.appendChild(description4);
